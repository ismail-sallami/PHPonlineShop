...
function deleteBills($bills) {
	global $mysqli;
	$i = 0;
	$stmt = $mysqli->prepare("DELETE FROM bills WHERE id = ?");
	$stmt2 = $mysqli->prepare("DELETE FROM bill_line WHERE bill_id = ?");
	foreach($bills as $id){
		$stmt->bind_param("i", $id);
		$stmt->execute();
		$stmt2->bind_param("i", $id);
...
function deleteCategory($category) {
	...
	$stmt2 = $mysqli->prepare("
							...
							FROM products AS t1
							JOIN category AS t2
							  ON t2.category_name = t1.product_cat
							Where t2.id = ? ");

	$stmt2->bind_param('i',$category);
	$stmt2->execute();

	$stmt1 = $mysqli->prepare("DELETE  FROM category  WHERE category.id = ?");
...
//Delete a defined array of product
function deleteProducts($products) {
	global $mysqli;
	$i = 0;
	$stmt = $mysqli->prepare("DELETE FROM products WHERE product_code = ?");
...
{
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("SELECT product_cat, product_code,product_name,price,product_qty FROM products");
	$stmt->execute();
	$stmt->bind_result($product_cat, $product_code, $product_name, $price, $product_qty);

	while ($stmt->fetch()){
		$row[] = array('product_cat' => $product_cat, 'product_code' => $product_code, 'product_name' => $product_name, 'price' => $price, 'product_qty' => $product_qty);
	}
	$stmt->close();
	return ($row);
}
...
{
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("SELECT * FROM bills");
	$stmt->execute();
	$stmt->bind_result($id, $date, $time, $firstname, $lastname, $email, $phone, $address1, $address2, $zip, $city, $country, $total, $status);

	while ($stmt->fetch()){
		$row[] = array('id' => $id, 'date' => $date, 'time' => $time, 'firstname' => $firstname, 'lastname' => $lastname, 'status' => $status);
	}
	$stmt->close();
	return ($row);
}
